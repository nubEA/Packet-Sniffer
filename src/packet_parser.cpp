...
void PacketParser::parse_ipv4(const std::vector<char>& data, size_t length, Packet& packet) {
    ...
    packet.ipv4Header.src_ip = ntohl(*reinterpret_cast<const uint32_t*>(ipv4_ptr + 12));
    packet.ipv4Header.dest_ip = ntohl(*reinterpret_cast<const uint32_t*>(ipv4_ptr + 16));
...
void PacketParser::parse_udp(const std::vector<char>& data, size_t length, Packet& packet) {
    ...
    uint16_t udpTotalLength = packet.udpHeader.length;
